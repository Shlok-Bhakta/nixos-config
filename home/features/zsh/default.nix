{ lib, config, ... }:
{
  programs.zsh = {
    ...
    shellAliases = {
      ...
      s = "sudo";
      tmuxhelp = ''
        echo "$(tput setaf 4)TMUX CHEATSHEET$(tput sgr0)
    ...
    initExtra = ''
      _sunshine_ipad() {
        local -a commands
        commands=(
          'start:Extend DP-1 and start Sunshine'
        ...
        _describe 'sunshine-ipad command' commands
      }

      compdef _sunshine_ipad sunshine-ipad
    '';
  };
}
